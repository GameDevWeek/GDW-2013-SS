...
public abstract class AbstractEnemy extends EntityCollidable {
    ...
    public void initPhysics() {
        ...
        createPhysics(BodyType.DYNAMIC, origin.x, origin.y).density(1)
                .friction(1).category(PhysixConst.ENEMY)
    ...
    public void render(GameContainer container, Graphics g)
        ...
        if (physicsObject != null && current_ani != null) {
            if (invertAni) {
                current_ani.draw(this.getPosition().x + current_ani.getWidth()
                        / 2,
                        this.getPosition().y - current_ani.getHeight() / 2,
                        -current_ani.getWidth(), current_ani.getHeight());
            } else {
                current_ani.draw(this.getPosition().x
                        - (current_ani.getWidth() / 2), this.getPosition().y
