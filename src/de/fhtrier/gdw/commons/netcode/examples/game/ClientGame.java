...
public class ClientGame extends BaseGame {
	...
	public void paint(Graphics g) {
		...
		// render the circle red when updates have been received, white
		// otherwise (messages have been deltified away by the server)
		if ((System.currentTimeMillis() - Client.lastMessage) < 50)
			g.setColor(Color.red);
		else
			g.setColor(Color.white);
		g.drawOval((int) player.position.x, (int) player.position.y, (int) 50,
				(int) 50);
	}
