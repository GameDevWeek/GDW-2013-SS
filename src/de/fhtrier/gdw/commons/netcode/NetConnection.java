...
public class NetConnection extends Thread {
    ...
    private ConcurrentLinkedQueue<INetDatagram> incomingDatagrams = new ConcurrentLinkedQueue<>();
    /** Outgoing datagrams unprocessed. */
    private ConcurrentLinkedQueue<INetDatagram> outgoingDatagrams = new ConcurrentLinkedQueue<>();
    ...
    /**
     * Create a connection to a client. This starts a thread that will receive
     * and prepare datagrams.
     * 
     * @param reception
     *            the reception that accepted the connection.
     * @param channel
     *            the socket channel to the client
     * @param datagramFactory
     *            the factory used to create datagrams based on their type.
     * @throws IOException
     *             when setting the TCP_NODELAY option fails.
     */
    NetConnection(NetReception reception, SocketChannel channel,
     ...
     * 
     * @param ip
     *            the server name or ip of the server to connect to
     * @param port
        ...
        try {
            for (Enumeration<NetworkInterface> en = NetworkInterface
                ...
                try {
                    ...
                    if (ifc.isUp()) {
                        for (Enumeration<InetAddress> ena = ifc
                                .getInetAddresses(); ena.hasMoreElements();) {
        ...
        while (!shutdown) {
            try {
                ...
                if (type == NetDatagram.Type.DISCONNECT) {
                    shutdown = true; // so we don't send the disconnect signal
                                     // back
                    shutdown();
                }
                ...
                if (!shutdown) {
                    // An exception causes a disconnect right now, maybe want to
                    // change that ?
     ...
     *            the datagram type
     * @param id
     *            a unique id, used in combo with type for identifying delta
     *            message bases
     * @param param1
     *            the first parameter of the header. If the message type is not
     *            none, this contains the message size.
     * @param param2
     *            the second parameter of the header. If the message type is not
     *            none, this contains the delta message size.
     * @throws IOException
     */
    private void handleDatagram(byte type, short id, short param1, short param2)
            throws IOException {
        INetDatagram datagram = datagramFactory.createDatagram(type, id,
                param1, param2);
        ...
        case DELTA:
            ...
            deltaMsg = (NetMessageDelta) NetMessageAllocator
                    .createMessageDelta();
            // If this is not the first message, we'll need the content of the
            // last received message.
            base = messageCacheIn.get(datagram.getType(), datagram.getID());
            // The new base will store the full (decompressed) content of this
            // message for use on the next message.
    ...
    public void sendPendingDatagrams() {
        if (!shutdown) {
            // If no message has been scheduled within the last x seconds, send
            // a keep alive signal.
            if (lastOutgoing != 0
                    && keepAliveTime > 0
                    && (System.currentTimeMillis() - lastOutgoing) > keepAliveTime) {
                sendEvent(INetDatagram.Type.KEEP_ALIVE, (short) 0, (short) 0,
                        (short) 0);
            }
            ...
            try {
                while (!outgoingDatagrams.isEmpty()) {
                    ...
                    case NONE:
                        ...
                        sendHeader(datagram.getType(), datagram.getID(),
                                datagram.getParam1(), datagram.getParam2());
                        break;
                    case NORMAL:
     ...
     * @throws IOException
     */
    private void sendHeader(byte type, short id, short param1, short param2)
            throws IOException {
        headerOut.clear();
        headerOut.put(type);
    ...
    private void sendMessageDelta(INetDatagram datagram) throws IOException {
        NetMessageDelta deltaMsg = (NetMessageDelta) NetMessageAllocator
                .createMessageDelta();
        // If this is not the first message, we'll need the content of the last
        // send message.
        NetMessage base = messageCacheOut.get(datagram.getType(),
                datagram.getID());
        // The new base will store the full (decompressed) content of this
        // message for use on the next message.
        ...
        // Send the header and both the message and the delta bits from the
        // channel
        sendHeader(datagram.getType(), datagram.getID(),
                (short) deltaMsg.position(), deltaMsg.deltaSize());
    ...
    public void shutdown() {
        try {
            if (!shutdown) {
                try {
                    sendHeader(NetDatagram.Type.DISCONNECT, (short) 0,
                            (short) 0, (short) 0);
                } catch (IOException e) {
    ...
    /**
     * Add a datagram to the pending datagrams queue. Note: If the connection
     * has not been accepted yet, it will do nothing.
     * 
     * @param datagram
    ...
    /**
     * Add an event datagram to the pending datagrams queue. Note: If the
     * connection has not been accepted yet, it will do nothing.
     * 
     * @param type
     *            the datagram type
     * @param id
    ...
    public void sendEvent(byte type, short id, short param1, short param2) {
        if (accepted) {
            lastOutgoing = System.currentTimeMillis();
            outgoingDatagrams
                    .add(new NetEventDatagram(type, id, param1, param2));
        }
    }
    ...
    /**
     * Set an attachment to this connection. This can be helpful to (for
     * example) store the player reference bound to this connection.
     * 
     * @param object
     ...
     * 
     * @param ms
     *            the time in ms since the last message has been send.
     */
    ...
    /**
     * Set the accepted state of this connection. If accepted is false, no new
     * datagrams will be scheduled to be send.
     * 
     * @param value
     *            the new value
     */
