 ...
 * 
 * Nice bonus: BitSet only creates enough bytes to store all bits. So if you
 * order the data from most changing to least changing, it will write very few
 * bytes for the deltaBits.
 * 
 * @author Lusito
 */
public class NetMessageDelta implements INetMessageInternal {
    ...
    /**
     * The maximum number of bytes the delta buffer can take. (number of
     * writing/reading operations = MAX_DELTA_SIZE * 8)
     */
    ...
    /**
     * The position in the deltaBits BitSet, or in other words the number of
     * write/read operations done so far.
     */
    ...
    /**
     * This is the last known state of this message. Used to compare to. null
     * when this is the first transmit of the message
     */
    private NetMessage base;
    /**
     * This will store the current values, which will be used the next time to
     ...
     * 
     * @param base
     *            the last known state of this message
     * @param newBase
     *            the message to store the current state in
     */
    ...
    public byte get() {
        ...
        if (base == null) {
            ...
            // If the delta bits say that it has changed since the last time,
            // read it fresh from the message.
        ...
        // Store the new value in newBase for use when this message arrives next
        // time.
        if (newBase != null) {
            newBase.put(value);
        }
    ...
    public void put(byte value) {
        ...
        if (base == null) {
            ...
            if (baseValue == value) {
                ...
                // If the value is not the same, note it in the delta bits and
                // write the new value to the message
